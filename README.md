...
- **Comprehensive RL Benchmark**: we provide the first bimanual manipulation task environment for RL, MARL, Multi-task RL, Meta RL, and Offline RL practitioners, along with a comprehensive benchmark for SOTA continuous control model-free RL/MARL methods. See [example](./dexhand/algorithms/marl/)
...
##### RL/MARL Examples
For example, if you want to train a policy for the ShadowHandOver task by the PPO algorithm, run this line in `dexhand` folder:
...
as an argument. For example, if you want to use happo algorithm, run this line in `dexhand` folder:
...
The training method of multi-task/meta RL is similar to the RL/MARL, it is only need to select the multi-task/meta categories and the corresponding algorithm. For example, if you want to train a policy for the ShadowHandMT4 categories by the MTPPO algorithm, run this line in `dexhand` folder:
...
Currently we only support PPO and PPO with LSTM methods in rl_games. If you want to use PPO with LSTM, run this line in `dexhand` folder:

```bash
python train_rlgames.py --task=ShadowHandOver --algo=ppo_lstm
``` 

The log files using rl_games can be found in `dexhand/runs` folder.
